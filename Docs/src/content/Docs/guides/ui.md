...
description: Every hotkey the mod adds and how to navigate its menus with a screen reader.
---

Every hotkey the mod adds, and what each key does inside the mod's menus. For how the underlying features behave, see [Game Mechanics](/SkyrimAccessibility/guides/mechanics/).

> Many of the keys you press during normal play (the nearby-object scanner on Page Up / Page Down / Home / End, autowalk on Shift + Home, the enemy locator on X, and the menu / journal reading) come from SkyrimTTS, not from this mod. SkyrimTTS is a required companion mod; see its [GUIDE.md](https://github.com/DioKyrie-Git/SkyrimTTS/blob/main/GUIDE.md) for that full key reference.
...
- **Clairvoyance**. Turn the auto-cast Clairvoyance guide spell on or off. See [Game Mechanics](/SkyrimAccessibility/guides/mechanics/) for what the spell does.
